...
// MultiExecutor 按数据源类型分发到 MySQL 或 Elasticsearch 执行器。
...
// NewMultiExecutor 创建多数据源执行器。MySQL 与 Elasticsearch 可为 nil，对应类型将返回 ErrUnsupportedDataSource。
func NewMultiExecutor(reg *datasource.Registry, mysql *MySQLExecutor, es *ESExecutor) *MultiExecutor {
	return &MultiExecutor{Registry: reg, MySQL: mysql, Elasticsearch: es}
}
